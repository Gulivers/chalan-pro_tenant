...
```bash
# En el VPS (raíz del repo = /opt/chalanpro)
cd /opt/chalanpro
git fetch origin
git checkout main
git pull origin main
# Opción recomendada: script idempotente (migraciones + collectstatic + restart)
sudo /opt/chalanpro/scripts/deploy-vps.sh
# O solo reiniciar si no hay cambios de código/build:
