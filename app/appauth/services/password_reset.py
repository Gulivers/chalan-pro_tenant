"""
Password reset helpers for JobRhythm authentication.
...
def frontend_base_url_for_password_reset(request):
    ...
    api_hosts = (
        {f'api.{tenant_base}', f'www.api.{tenant_base}'} if tenant_base else set()
    )
...
def apply_password_reset(user, new_password):
    """
    Persist a new password and invalidate all auth tokens for the user.
    Caller must verify the reset token first.
    """
    user.set_password(new_password)
    user.save(update_fields=['password'])
    invalidate_user_auth_tokens(user)
