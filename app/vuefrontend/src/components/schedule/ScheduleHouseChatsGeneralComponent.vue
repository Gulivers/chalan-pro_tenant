<template>
  <div class="container-fluid">
    <div class="row">
      <!-- Sidebar de eventos -->
      <div class="col-md-5 col-lg-4 col-xl-3 border-end p-2 bg-light">
        <div class="mb-2 border-bottom text-center">
          <h4 class="mb-0 text-muted">Work Orders</h4>
        </div>
        <div class="row justify-content-end" ref="leftHeader">
          <div class="text-end">
        ...
        <ul
          class="list-group events-list"
          ref="events"
          :style="eventsMaxHeightStyle">
          <li
            ...
            <div
              ...
              <span
                class="badge bg-secondary text-capitalize ms-2"
            ...
            <small
              class="text-muted d-flex align-items-center justify-content-between overflow-hidden"
              style="font-size: 0.75rem; gap: 0.75rem">
              <span class="text-truncate" style="max-width: 25%">
                {{ event.crew_title }}
              </span>
              <span
                v-if="event.extended_service"
      ...
      <!-- Chat window -->
      <div
        class="col-md-7 col-lg-8 col-xl-9 d-flex flex-column p-1"
        :style="contentMaxHeightStyle">
        <div
          v-if="selectedEvent"
          class="chat-window border rounded p-1 d-flex flex-column flex-grow-1">
          <ScheduleHouseDiscussion
            v-if="selectedEvent"
            :event="selectedEvent" />
        </div>
        <div
          class="d-flex flex-column align-items-center justify-content-center h-100 gap-3"
          v-if="loading">
          <div class="spinner-border text-primary" role="status">
            <span class="visually-hidden">Loading...</span>
          </div>
          <h2 class="text-center mb-0">Preparing Work Order...</h2>
        </div>
        <div
          class="d-flex align-items-center h-100"
          v-if="!loading && !selectedEvent">
          <h2 class="text-center w-100">Please select an Work Order first.</h2>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    async getEvents(event, page = 1) {
      this.page = page;
      const currentSelectedId = this.selectedEvent
        ? this.selectedEvent.id
        : null;

      try {
        const url_get = `/api/my-events/?page=${this.page}${
        ...
        if (response.status === 200) {
          ...
          if (currentSelectedId) {
            const stillVisible = this.events.find(
    ...
    async fetchOnlyEvents(page = 1) {
      try {
        const url_get = `/api/my-events/?page=${page}${
          this.search.trim() ? "&search=" + this.search : ""
        }`;
        const response = await axios.get(url_get);
    ...
    connectUnreadWebSocket() {
      ...
      this.wsNotify.onmessage = (event) => {
        ...
        if (data.type === "unread.updated") {
          ...
          if (
            this.selectedEvent === null ||
            this.selectedEvent.id !== event_id
          ) {
