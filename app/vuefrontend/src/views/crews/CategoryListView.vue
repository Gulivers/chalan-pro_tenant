<template>
  <JRPage>
    <JRPageHeader title="Crew Categories">
      <template #actions>
        <JRButton
    ...
    <JRToolbar>
      <template #start>
        <div class="jr-master-list__search">
          <label class="jr-sr-only" for="cat-filter-input">
            Search crew categories
          </label>
          <span class="jr-master-list__search-icon" aria-hidden="true">
            <SearchIcon />
          </span>
          <JRInput
            inputId="cat-filter-input"
            v-model="filter"
            type="search"
      ...
      <template #actions>
        <label class="jr-sr-only" for="cat-per-page">Entries per page</label>
        <JRSelect
          class="jr-master-list__entries"
          inputId="cat-per-page"
          v-model="perPage"
          :options="pageOptions"
          optionLabel="label"
          optionValue="value" />
      ...
      <ul
        ...
        <li v-for="item in pagedItems" :key="item.id" class="jr-master-row">
          <div class="jr-master-row__main">
            ...
            <p class="jr-master-row__meta">ID {{ item.id }}</p>
          </div>
...
const emptyTitle = computed(() => {
  if (loadError.value) return "Could not load crew categories";
  if (hasSearch.value) return "No matching crew categories";
  return "No crew categories";
});
const emptyDescription = computed(() => {
  if (loadError.value) return "Try Refresh.";
  const query = filter.value.trim();
  if (query) return `No crew categories match “${query}”.`;
  return "No crew categories yet.";
});

const clearSearch = () => {
  filter.value = "";
  if (typeof document !== "undefined") {
    document.getElementById("cat-filter-input")?.focus();
  }
};

const canView = computed(() =>
  !!proxy?.hasPermission?.("crewsapp.view_category")
);
const hasRowActions = computed(
  () =>
    canView.value ||
    !!proxy?.hasPermission?.("crewsapp.change_category") ||
    !!proxy?.hasPermission?.("crewsapp.delete_category")
);
...
const viewTo = (id) => `/crews/categories/view/${id}`;
const editTo = (id) => `/crews/categories/edit/${id}`;
const goToCreateForm = () => router.push("/crews/categories/form");
const viewItem = (id) => router.push(viewTo(id));
const editItem = (id) => router.push(editTo(id));

const getRowActions = (item) => {
  const actions = [];
  if (proxy?.hasPermission?.("crewsapp.view_category")) {
    actions.push({
      key: "view",
      label: "View",
      severity: "success",
  ...
  if (proxy?.hasPermission?.("crewsapp.change_category")) {
    actions.push({
      key: "edit",
      label: "Edit",
      severity: "primary",
  ...
  if (proxy?.hasPermission?.("crewsapp.delete_category")) {
    actions.push({
      key: "delete",
      label: "Delete",
      severity: "danger",
