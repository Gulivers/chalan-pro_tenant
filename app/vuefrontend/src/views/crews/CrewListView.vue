  ...
  <JRPage>
    ...
    <JRToolbar>
      <template #start>
        <div class="jr-master-list__search">
          <label class="jr-sr-only" for="crew-filter-input">
            Search crews
          </label>
      ...
      <template #actions>
        <label class="jr-sr-only" for="crew-per-page">Entries per page</label>
        <JRSelect
          class="jr-master-list__entries"
          inputId="crew-per-page"
          v-model="perPage"
          :options="pageOptions"
          optionLabel="label"
          optionValue="value" />
        ...
        <li v-for="item in pagedItems" :key="item.id" class="jr-master-row">
          <div class="jr-master-row__main">
            ...
            <p class="jr-master-row__meta">
              ...
              ·
              {{
                item.permission_create_event
                  ? "Can schedule"
                  : "No schedule permission"
              }}
            </p>
          </div>
    ...
    <div v-else class="jr-master-list__table">
      <JRDataTable
        ...
        <Column field="name" header="Crew Name" sortable>
          <template #body="{ data }">
            <router-link
              v-if="canView"
              class="jr-master-row__name jr-master-row__name--link"
        ...
        <Column
          field="permission_create_event"
          header="Can Create/Update Schedule?"
          sortable>
          <template #body="{ data }">
            <JRBadge
              :value="data.permission_create_event ? 'Yes' : 'No'"
              :severity="
                data.permission_create_event ? 'success' : 'secondary'
              " />
          </template>
        </Column>
        <Column
...
const emptyTitle = computed(() => {
  if (loadError.value) return "Could not load crews";
  if (hasSearch.value) return "No matching crews";
  return "No crews";
});
const emptyDescription = computed(() => {
  if (loadError.value) return "Try Refresh.";
  const query = filter.value.trim();
  if (query) return `No crews match “${query}”.`;
  return "No crews yet.";
});
...
const clearSearch = () => {
  ...
  if (typeof document !== "undefined") {
    document.getElementById("crew-filter-input")?.focus();
  }
};

const canView = computed(() => !!proxy?.hasPermission?.("crewsapp.view_crew"));
const hasRowActions = computed(
  () =>
    canView.value ||
    !!proxy?.hasPermission?.("crewsapp.change_crew") ||
    !!proxy?.hasPermission?.("crewsapp.delete_crew")
);
...
const viewTo = (id) => `/crews/view/${id}`;
const editTo = (id) => `/crews/edit/${id}`;
const goToCreateForm = () => router.push("/crews/form");
const viewItem = (id) => router.push(viewTo(id));
const editItem = (id) => router.push(editTo(id));

const getRowActions = (item) => {
  const actions = [];
  if (proxy?.hasPermission?.("crewsapp.view_crew")) {
    actions.push({
      key: "view",
      label: "View",
      severity: "success",
  ...
  if (proxy?.hasPermission?.("crewsapp.change_crew")) {
    actions.push({
      key: "edit",
      label: "Edit",
      severity: "primary",
  ...
  if (proxy?.hasPermission?.("crewsapp.delete_crew")) {
    actions.push({
      key: "delete",
      label: "Delete",
      severity: "danger",
