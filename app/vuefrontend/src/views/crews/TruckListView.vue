  ...
  <TxCard class="shadow-sm mt-0">
    <template #header>
      <div class="d-flex justify-content-between align-items-center w-100">
        <h6 class="text-primary mb-0">Trucks</h6>
        <router-link
          v-if="hasPermission('crewsapp.add_truck')"
          to="/crews/trucks/form"
          class="btn btn-success"
          >+ New Truck</router-link
        >
      </div>
    </template>
      ...
      <template #cell(actions)="data">
        <td class="text-center">
          <div class="btn-group btn-group-sm" role="group">
            <router-link
              v-if="hasPermission('crewsapp.view_truck')"
              :to="`/crews/trucks/view/${data.item.id}`"
              class="btn btn-outline-success me-1"
              >View</router-link
            >
            <router-link
              v-if="hasPermission('crewsapp.change_truck')"
              :to="`/crews/trucks/edit/${data.item.id}`"
              class="btn btn-outline-primary me-1"
              >Edit</router-link
            >
            <button
              v-if="hasPermission('crewsapp.delete_truck')"
...
const fields = [
  ...
  { key: "plate_number", label: "Plate Number", sortable: true, thClass: "text-start", tdClass: "text-start" },
  { key: "model", label: "Model", sortable: true, thClass: "text-start", tdClass: "text-start" },
  { key: "year", label: "Year", sortable: true, thClass: "text-center", tdClass: "text-center" },
  {
    key: "status",
    label: "Status",
    thClass: "text-center",
