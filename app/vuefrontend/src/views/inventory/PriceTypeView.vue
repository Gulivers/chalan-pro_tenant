<template>
  <JRPage>
    <JRPageHeader title="Price Types">
      <template #actions>
        <JRButton
    ...
    <JRToolbar>
      <template #start>
        <div class="jr-master-list__search">
          <label class="jr-sr-only" for="price-type-filter-input">
            Search price types
          </label>
          ...
          <JRInput
            inputId="price-type-filter-input"
            v-model="filter"
            type="search"
      ...
      <template #actions>
        <label class="jr-sr-only" for="price-type-per-page">
          Entries per page
        </label>
        <JRSelect
          class="jr-master-list__entries"
          inputId="price-type-per-page"
          v-model="perPage"
          :options="pageOptions"
          optionLabel="label"
          optionValue="value" />
        ...
        <li v-for="item in items" :key="item.id" class="jr-master-row">
          <div class="jr-master-row__main">
            ...
            <p class="jr-master-row__meta">
              <span>{{ pricingMethodLabel(item.pricing_method) }}</span>
              <span aria-hidden="true">·</span>
              <span>{{
                formatMarginPercent(item.margin_percent, item.pricing_method)
              }}</span>
            </p>
          </div>
    ...
    <div v-else class="jr-master-list__table">
      <JRDataTable
        ...
        scrollable
        stripedRows
        :tableStyle="isTablet ? 'min-width: 40rem' : 'min-width: 48rem'"
        ...
        <Column v-if="!isTablet" field="description" header="Description" sortable>
          <template #body="{ data }">
            {{ data.description || "—" }}
          </template>
        </Column>
        <Column field="pricing_method" header="Pricing method" sortable>
          <template #body="{ data }">
            {{ pricingMethodLabel(data.pricing_method) }}
          </template>
        </Column>
        <Column
          ...
          <template #body="{ data }">
            {{ formatMarginPercent(data.margin_percent, data.pricing_method) }}
          </template>
        </Column>
...
export default {
  ...
  setup() {
    ...
    const emptyTitle = computed(() =>
      loadError.value ? "Could not load price types" : "No price types"
    );
    const emptyDescription = computed(() =>
      loadError.value
        ? "Try Refresh."
        : "No price types match the current search."
    );

    const canView = computed(() =>
      !!proxy?.hasPermission?.("appinventory.view_pricetype")
    );
    const hasRowActions = computed(
      () =>
        canView.value ||
        !!proxy?.hasPermission?.("appinventory.change_pricetype") ||
        !!proxy?.hasPermission?.("appinventory.delete_pricetype")
    );
    ...
    const formatMarginPercent = (marginPercent, pricingMethod) => {
      ...
      if (pricingMethod === "NONE" || !pricingMethod) return "—";
    ...
    const viewTo = (id) => ({ name: "price-type-view", params: { id } });
    ...
    const getRowActions = (item) => {
      const actions = [];
      if (proxy?.hasPermission?.("appinventory.view_pricetype")) {
        actions.push({
          key: "view",
          label: "View",
          severity: "success",
      ...
      if (proxy?.hasPermission?.("appinventory.change_pricetype")) {
        actions.push({
          key: "edit",
          label: "Edit",
          severity: "primary",
      ...
      if (proxy?.hasPermission?.("appinventory.delete_pricetype")) {
        actions.push({
          key: "delete",
          label: "Delete",
          severity: "danger",
