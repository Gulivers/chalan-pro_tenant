<template>
  <JRPage>
    <JRPageHeader title="Unit of Measure">
      <template #actions>
        <JRButton
          v-if="hasPermission('appinventory.add_unitofmeasure')"
          type="button"
          :fluid="isMobile"
    ...
    <JRToolbar>
      <template #start>
        <div class="jr-master-list__search">
          <label class="jr-sr-only" for="uom-filter-input">
            Search units
          </label>
      ...
      <template #actions>
        <label class="jr-sr-only" for="uom-per-page">Entries per page</label>
        <JRSelect
          class="jr-master-list__entries"
          inputId="uom-per-page"
          v-model="perPage"
          :options="pageOptions"
          optionLabel="label"
          optionValue="value" />
    ...
    <div v-else class="jr-master-list__table">
      <JRDataTable
        ...
        scrollable
        stripedRows
        :tableStyle="isTablet ? 'min-width: 40rem' : 'min-width: 52rem'"
        ...
        <Column v-if="!isTablet" field="reference_unit" header="Ref. unit" sortable>
          <template #body="{ data }">
            {{ data.reference_unit ? "Yes" : "No" }}
          </template>
        </Column>
        <Column
          v-if="!isTablet"
          field="conversion_sign"
          header="Sign"
          sortable />
        <Column
          v-if="!isTablet"
...
const ENDPOINT = "/api/unitsofmeasure-provider/";
...
export default {
  ...
  setup() {
    ...
    const emptyTitle = computed(() =>
      loadError.value ? "Could not load units" : "No units"
    );
    const emptyDescription = computed(() =>
      loadError.value ? "Try Refresh." : "No units match the current search."
    );

    const canView = computed(() =>
      !!proxy?.hasPermission?.("appinventory.view_unitofmeasure")
    );
    const hasRowActions = computed(
      () =>
        canView.value ||
        !!proxy?.hasPermission?.("appinventory.change_unitofmeasure") ||
        !!proxy?.hasPermission?.("appinventory.delete_unitofmeasure")
    );
    ...
    const getOrderingFromSortBy = (sortBy) => {
      ...
      if (!field) return "-id";
      if (field === "category_name") field = "category__name";
    ...
    const loadItems = async () => {
      ...
      try {
        ...
        proxy?.notifyError?.("Error loading units.");
      } finally {
        setTimeout(() => {
    ...
    const viewTo = (id) => ({ name: "unit-measure-view", params: { id } });
    const goToCreateForm = () => router.push({ name: "unit-measure-form" });
    const viewItem = (id) => router.push(viewTo(id));
    const editItem = (id) =>
      router.push({ name: "unit-measure-edit", params: { id } });

    const getRowActions = (item) => {
      const actions = [];
      if (proxy?.hasPermission?.("appinventory.view_unitofmeasure")) {
        actions.push({
          key: "view",
          label: "View",
          severity: "success",
      ...
      if (proxy?.hasPermission?.("appinventory.change_unitofmeasure")) {
        actions.push({
          key: "edit",
          label: "Edit",
          severity: "primary",
      ...
      if (proxy?.hasPermission?.("appinventory.delete_unitofmeasure")) {
        actions.push({
          key: "delete",
          label: "Delete",
          severity: "danger",
    ...
    const deleteItem = (id) => {
      proxy?.confirmDelete?.(
        "Delete?",
        "This will delete the unit. This action cannot be undone.",
        async () => {
          try {
            await axios.delete(`/api/unitsofmeasure/${id}/`);
            proxy?.notifyToastSuccess?.("The unit has been deleted.");
            refreshTable();
          } catch (error) {
            ...
            if (status === 403) {
              ...
              proxy?.notifyError?.(
                data?.detail || "Cannot delete: unit is in use."
              );
            } else {
              proxy?.notifyError?.(data?.detail || "Error deleting the unit.");
            }
          }
        }
      );
