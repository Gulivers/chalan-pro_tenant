      ...
      <BOverlay :show="isLoading" rounded="sm" opacity="0.85" variant="light">
        <template #overlay>
          <div class="text-center">
            ...
            <div class="h5 text-primary">Loading Categories...</div>
        ...
        <BTable
          ...
          <template #cell(actions)="row">
            <div class="btn-group btn-group-sm">
              <button
                v-if="hasPermission('appinventory.view_productcategory')"
                class="btn btn-outline-success me-1"
                @click="viewItem(row.item.id)">
                View
              </button>
              <button
                v-if="hasPermission('appinventory.change_productcategory')"
                class="btn btn-outline-primary me-1"
                @click="editItem(row.item.id)">
                Edit
              </button>
              <button
                v-if="hasPermission('appinventory.delete_productcategory')"
...
export default {
  ...
  setup() {
    ...
    const viewItem = (id) =>
      router.push({ name: "product-category-view", params: { id } });
    const editItem = (id) =>
