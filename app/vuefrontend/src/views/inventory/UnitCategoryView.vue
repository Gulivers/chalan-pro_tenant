<template>
  <JRPage>
    <JRPageHeader title="Unit Categories">
      <template #actions>
        <JRButton
          v-if="hasPermission('appinventory.add_unitcategory')"
          type="button"
          :fluid="isMobile"
    ...
    <JRToolbar>
      <template #start>
        <div class="jr-master-list__search">
          <label class="jr-sr-only" for="unit-category-filter-input">
          ...
          <JRInput
            inputId="unit-category-filter-input"
            v-model="filter"
            type="search"
            placeholder="Search unit categories..." />
        </div>
      </template>
      ...
      <template #actions>
        <label class="jr-sr-only" for="unit-category-per-page">
          Entries per page
        </label>
        <JRSelect
          class="jr-master-list__entries"
          inputId="unit-category-per-page"
          v-model="perPage"
          :options="pageOptions"
          optionLabel="label"
          optionValue="value" />
...
const ENDPOINT = "/api/unitcategory-provider/";
...
export default {
  ...
  setup() {
    ...
    const emptyTitle = computed(() =>
      loadError.value ? "Could not load unit categories" : "No unit categories"
    );
    const emptyDescription = computed(() =>
      loadError.value
        ? "Try Refresh."
        : "No unit categories match the current search."
    );

    const canView = computed(() =>
      !!proxy?.hasPermission?.("appinventory.view_unitcategory")
    );
    const hasRowActions = computed(
      () =>
        canView.value ||
        !!proxy?.hasPermission?.("appinventory.change_unitcategory") ||
        !!proxy?.hasPermission?.("appinventory.delete_unitcategory")
    );
    ...
    const loadItems = async () => {
      ...
      try {
        ...
        proxy?.notifyError?.("Error loading unit categories.");
      } finally {
        setTimeout(() => {
    ...
    const viewTo = (id) => ({ name: "unit-category-view", params: { id } });
    const goToCreateForm = () => router.push({ name: "unit-category-form" });
    const viewItem = (id) => router.push(viewTo(id));
    const editItem = (id) =>
      router.push({ name: "unit-category-edit", params: { id } });

    const getRowActions = (item) => {
      const actions = [];
      if (proxy?.hasPermission?.("appinventory.view_unitcategory")) {
        actions.push({
          key: "view",
          label: "View",
          severity: "success",
      ...
      if (proxy?.hasPermission?.("appinventory.change_unitcategory")) {
        actions.push({
          key: "edit",
          label: "Edit",
          severity: "primary",
      ...
      if (proxy?.hasPermission?.("appinventory.delete_unitcategory")) {
        actions.push({
          key: "delete",
          label: "Delete",
          severity: "danger",
    ...
    const deleteItem = (id) => {
      proxy?.confirmDelete?.(
        "Delete?",
        "This will delete the unit category. This action cannot be undone.",
        async () => {
          try {
            await axios.delete(`/api/unitcategory/${id}/`);
            proxy?.notifyToastSuccess?.("The unit category has been deleted.");
            refreshTable();
          } catch (error) {
            ...
            if (status === 403) {
              ...
              proxy?.notifyError?.(
                data?.detail || "Error deleting the unit category."
              );
            }
          }
        }
